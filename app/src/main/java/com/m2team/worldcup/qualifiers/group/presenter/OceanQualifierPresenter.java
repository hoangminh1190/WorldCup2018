...
public class OceanQualifierPresenter extends Presenter {


    public static String LINK = "http://www.fifa.com/worldcup/preliminaries/oceania/index.html";

    private OnDataCompleteListener listener;
    private Context mContext;

    public OceanQualifierPresenter(Context context) {
        mContext = context;
    }

    public void getData() {
        getData(mContext, Common.OCEAN_GROUPS_QUALIFIER, new QualifierSubscriber(), Common.ONE_DAY_IN_MILLISECONDS);
    }
    ...
    public Observable<List<Group>> getFromCache() {
        return Observable.create(new Observable.OnSubscribe<List<Group>>() {
            ...
            public void call(Subscriber<? super List<Group>> subscriber) {
                Gson gson = new Gson();
                String json = Common.getPrefString(mContext, Common.OCEAN_GROUPS_QUALIFIER, Common.KEY_JSON_DATA);
    ...
    public Observable<List<Group>> getFromServer() {
        return Observable.create(new Observable.OnSubscribe<List<Group>>() {
            ...
            public void call(List<Group> groups) {
                ...
                Common.putPrefValue(mContext, Common.OCEAN_GROUPS_QUALIFIER, Common.KEY_JSON_DATA, json);
                Common.putPrefValue(mContext, Common.OCEAN_GROUPS_QUALIFIER, Common.KEY_JSON_EXPIRED, System.currentTimeMillis());
